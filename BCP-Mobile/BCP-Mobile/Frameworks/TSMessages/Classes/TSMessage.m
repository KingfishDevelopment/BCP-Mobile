                     ...
                     canBeDismisedByUser:(BOOL)dismissingEnabled
{
    ...
    void (^completionBlock)(BOOL finished) = ^(BOOL finished){
        ...
        [weakSelf prepareNotificationToBeShown:v];
    };
    
    //fade out existing TSMessageView
    TSMessageView *currentNotification = (TSMessageView *)[[BCPCommon viewController] currentNotification];
    if(currentNotification) {
        CGPoint fadeOutToPoint;
        if (currentNotification.messagePosition == TSMessageNotificationPositionTop)
        {
            fadeOutToPoint = CGPointMake(currentNotification.center.x, -CGRectGetHeight(currentNotification.frame)/2.f);
        }
        else
        {
            fadeOutToPoint = CGPointMake(currentNotification.center.x,
                                         currentNotification.viewController.view.bounds.size.height + CGRectGetHeight(currentNotification.frame)/2.f);
        }
        
        [UIView animateWithDuration:kTSMessageAnimationDuration animations:^
         {
             currentNotification.center = fadeOutToPoint;
             if (![TSMessage iOS7StyleEnabled]) {
                 currentNotification.alpha = 0.f;
             }
...
- (void)fadeInCurrentNotification
{
    ...
    {
        int BCPNavigationBarHeight = [((NSObject<BCPViewControllerDelegate> *)currentView.viewController) navigationBarHeight];
        if(BCPNavigationBarHeight>0) {
            verticalOffset+=BCPNavigationBarHeight;
            [((NSObject<BCPViewControllerDelegate> *)currentView.viewController) insertSubviewBelowNavigationBar:currentView];
            if ([TSMessage iOS7StyleEnabled]) {
                //addStatusBarHeightToVerticalOffset();
            }
        }
